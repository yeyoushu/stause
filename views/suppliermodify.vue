<template>
  ...
  <el-breadcrumb-item>采购单编辑</el-breadcrumb-item>
    ...
    <el-button type="success" @click="additemss">保存</el-button>
    ...
    <el-form ref="form" :inline="true" label-width="80px">
      ...
      <div v-for="(item, index) in pomain.poitems" :key="index" class="sdsasd">
        ...
        <el-form-item :prop="'poitems.' + index + '.itemPrice'" class="sdsasda">
          <el-input :value="item.num*item.unitPrice"  v-model="item.itemPrice"></el-input>
        </el-form-item>
...
      },
  ...
  methods: {
    //   获取指定采购单的明细
       updata() {
      axios
        .get("cgi/main/purchase/pomain/queryItem?poId=" + this.$route.params.id)
        .then((value) => {
          console.log(value.data);
          this.pomain.poitems = value.data;
        });
    },
    //   添加明细显示
    additems() {
    ...
    // 提交保存
    additemss(){
        // var b=pomain.poitems.length;
        // for (var i=0;i<b;i++){
        //    this.pomain.poitems.itemPrice=parseInt(this.pomain.poitems[i].unitPrice)*parseInt(this.pomain.poitems[i].num)
        // }  
...
axios({
    url: '/cgi/main/purchase/pomain/update',
    method: 'post',
    ...
    handleEdit(index, row) {
      ...
      console.log(this.pomain.poitems[this.currentIndex]);
    },
...
.placeres {background-color: rgb(247, 189, 151);border: solid 1px rgb(247, 189, 151);color: #000;}
</style>