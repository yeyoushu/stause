<template>
  ...
  <el-breadcrumb-item>新添销售单</el-breadcrumb-item>
</el-breadcrumb>
    <!-- 增加部分开始 -->
     <div class="place">
      <el-button type="info"  @click="dialog"
        class="places"><span>添加销售单</span></el-button
      >
    </div>
    ...
    <template>
      <el-table :data="product" style="width: 100%">
        ...
        <el-table-column label="销售单编号" width="140">
          <template slot-scope="scope">
        ...
        <el-table-column label="销售单总价" width="110">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getMessage(currentPage) {
       this.type=1
      axios.get("/cgi/main/sell/somain/show?type=1&page="+this.currentPage).then((value) => {
  console.log(value.data.list);
    ...
    handleEditl(index, row) {
        this.$router.push("/salesedit/"+row.soId+ '/'+JSON.stringify(row) );
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除吗?", "提示", {
        ...
        .then(() => {
          ...
          axios
            .post(
              "/cgi/main/sell/somain/delete?soId=" + row.soId
            )
            .then((response) => {
    ...
    changes() {
      ...
      axios
        .post(
          "cgi/main/purchase/vender/update?" + querystring.stringify(this.change)
        )
        .then((value) => {
          console.log(value);
           this.getMessage();
